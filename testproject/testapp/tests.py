...
from django_adx.utils import build_dhis2_id
from django.test import TestCase
from django.db.models import Q, Count
from .models import Gender, Insuree, HealthFacility
...
class ADXTests(TestCase):
    ...
    AGE_CATEGORY_DEFINITION = ADXMappingCategoryDefinition(
        ...
        category_options=[
            ADXCategoryOptionDefinition(
                code="P0Y50Y", name="P0Y50Y", filter=Q(dob__gte=_50_YEARS_AGO)
            ),
            ADXCategoryOptionDefinition(
                code="P50Y9999Y", name="P50Y9999Y", filter=Q(dob__lt=_50_YEARS_AGO)
            ),
        ],
    )
            ...
            ADXMappingGroupDefinition(
                ...
                data_values=[
                    ADXMappingDataValueDefinition(
                        ...
                        categories=[
                            AGE_CATEGORY_DEFINITION,
                            SEX_CATEGORY_DEFINITION
                        ],
                    )
                ],
            )
    ...
    def test_adx_mapping(self):
        ...
        self.assertEqual(find_value_by_attributes(xml_format, {"AGEGROUP": 'P0Y50Y', "SEX": 'M'}), '1')
        self.assertEqual(find_value_by_attributes(xml_format, {"AGEGROUP": 'P0Y50Y', "SEX": 'F'}), '2')
        self.assertEqual(find_value_by_attributes(xml_format, {"AGEGROUP": 'P0Y50Y', "SEX": 'O'}), '1')
        self.assertEqual(find_value_by_attributes(xml_format, {"AGEGROUP": 'P50Y9999Y', "SEX": 'M'}), None)
        self.assertEqual(find_value_by_attributes(xml_format, {"AGEGROUP": 'P50Y9999Y', "SEX": 'F'}), None)
        self.assertEqual(find_value_by_attributes(xml_format, {"AGEGROUP": 'P50Y9999Y', "SEX": 'O'}), None)        
...
git remo        self._test_datavalue(asdict(adx_format), self.EXPECTED_ADX_DICT)
    ...
    def _create_test_organization_unit(cls):
        cls.dateTest = datetime.datetime.now()
        Gender.objects.create(code="M", name="Male")
        Gender.objects.create(code="F", name="Female")
        Gender.objects.create(code="O", name="Other")
        # First valid district
        cls._TEST_HF = HealthFacility.objects.create()
        cls._TEST_HF.refresh_from_db()
        cls._TEST_INSUREES = [
        ...
        cls.EXPECTED_ADX_DICT = {
            ...
            "exported": cls.dateTest,
            "groups": [
                {
                    "complete_date": cls.dateTest.strftime("%Y-%m-%d"),
                    "org_unit": org_unit,
                    ...
                    "data_values": [
                        {
                            ...
                            "value": "1",
                            "aggregations": [
                                {"label_name": "AGEGROUP", "label_value": "P0Y50Y"},
                        ...
                        },
                        {
                            ...
                            "value": "1",
                            "aggregations": [
                                {"label_name": "AGEGROUP", "label_value": "P50Y9999Y"},
                                {"label_name": "SEX", "label_value": "M"},
                            ],
                        },
                    ],
...
def find_value_by_attributes(root, attributes):
    # XPath to find the correct element
    ns = {'ns': 'urn:ihe:qrph:adx:2015'}
    xpath = ".//dataValue"
